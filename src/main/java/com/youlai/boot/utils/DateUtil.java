...
import com.youlai.boot.system.model.vo.DateLimitUtilVo;
...
public class DateUtil {
    ...
    public static String nowDate() {
        return nowDate(SysConfigConstant.DATE_FORMAT_NUM);
    }
    ...
    public static Date nowDateTime() {
        return strToDate(nowDateTimeStr(), SysConfigConstant.DATE_FORMAT);
    }
    ...
    public static String nowDateTimeStr() {
        return nowDate(SysConfigConstant.DATE_FORMAT);
    }
    ...
    public static String addDay(Date newDate, int num) {
        SimpleDateFormat format = new SimpleDateFormat(SysConfigConstant.DATE_FORMAT);
        Calendar ca = Calendar.getInstance();
        ca.setTime(newDate);
        ca.add(Calendar.DATE, num);
        return format.format(ca.getTime());
    ...
    public static long getTwoDateDays(Date beforeDay, Date afterDay) {
        SimpleDateFormat sm = new SimpleDateFormat(SysConfigConstant.DATE_FORMAT_NUM);
        long days = -1;
        try {
    ...
    public static int getSecondTimestamp(String date){
        ...
        Date date1 = strToDate(date, SysConfigConstant.DATE_FORMAT);
        if(date1 == null){
            return 0;
        }
    ...
    public static int getSecondTimestamp(){
        Date date = strToDate(nowDateTime(SysConfigConstant.DATE_FORMAT), SysConfigConstant.DATE_FORMAT);
        if (null == date) {
            return 0;
        }
        String timestamp = String.valueOf(date.getTime()/1000);
    ...
    public static String getWeekEndDay() {
        return addDay(getWeekStartDay(), 7, SysConfigConstant.DATE_FORMAT);
    }
    ...
    public static String getLastWeekEndDay() {
        return addDay(getLastWeekStartDay(), 7, SysConfigConstant.DATE_FORMAT);
    }
    ...
    public static DateLimitUtilVo getDateLimit(String data){
        ...
        String endTime = DateUtil.nowDateTime(SysConfigConstant.DATE_FORMAT);
        String day = DateUtil.nowDateTime(SysConfigConstant.DATE_FORMAT_START);
        String end = DateUtil.nowDateTime(SysConfigConstant.DATE_FORMAT_END);

        if(!StringUtils.isBlank(data)){
            switch (data){
                case SysConfigConstant.SEARCH_DATE_DAY:
                    startTime = day;
                    break;
                case SysConfigConstant.SEARCH_DATE_YESTERDAY:
                    startTime = DateUtil.addDay(day, -1, SysConfigConstant.DATE_FORMAT_START);
                    endTime = DateUtil.addDay(end, -1, SysConfigConstant.DATE_FORMAT_END);
                    break;
                case SysConfigConstant.SEARCH_DATE_LATELY_7:
                    startTime = DateUtil.addDay(day, -6, SysConfigConstant.DATE_FORMAT_START);
                    break;
                case SysConfigConstant.SEARCH_DATE_WEEK:
                    startTime = getWeekStartDay();
                    endTime = getWeekEndDay();
                    break;
                case SysConfigConstant.SEARCH_DATE_PRE_WEEK:
                    startTime = getLastWeekStartDay();
                    endTime = getLastWeekEndDay();
                    break;
                case SysConfigConstant.SEARCH_DATE_LATELY_30:
                    startTime = DateUtil.addDay(day, -30, SysConfigConstant.DATE_FORMAT_START);
                    break;
                case SysConfigConstant.SEARCH_DATE_MONTH:
                    startTime = DateUtil.nowDateTime(SysConfigConstant.DATE_FORMAT_MONTH_START);
                    endTime = getMonthEndDay();
                    break;
                case SysConfigConstant.SEARCH_DATE_PRE_MONTH:
                    startTime = getLastMonthStartDay();
                    endTime = getLastMonthEndDay();
                    break;
                case SysConfigConstant.SEARCH_DATE_YEAR:
                    startTime = DateUtil.nowDateTime(SysConfigConstant.DATE_FORMAT_YEAR_START);
                    endTime = DateUtil.nowDateTime(SysConfigConstant.DATE_FORMAT_YEAR_END);
                    break;
                case SysConfigConstant.SEARCH_DATE_PRE_YEAR:
                    startTime = getLastYearStartDay();
                    endTime = getLastYearEndDay();
                    break;
                default:
                    List<String> list = MrZhangUtil.stringToArrayStr(data);
                    if(list.size() == 1){
                        throw new UsdtException("选择时间参数格式错误，请在 " +
                                SysConfigConstant.SEARCH_DATE_DAY + "|" +
                                SysConfigConstant.SEARCH_DATE_YESTERDAY + "|" +
                                SysConfigConstant.SEARCH_DATE_LATELY_7 + "|" +
                                SysConfigConstant.SEARCH_DATE_LATELY_30 + "|" +
                                SysConfigConstant.SEARCH_DATE_MONTH + "|" +
                                SysConfigConstant.SEARCH_DATE_YEAR + "|自定义时间范围（格式：yyyy-MM-dd HH:mm:ss，两个时间范围用逗号分割）");
                    }
                    startTime = list.get(0);
                    endTime = list.get(1);

//                    if (startTime.equals(endTime)) {
                    startTime = DateUtil.appointedDayStrToFormatStr(startTime, SysConfigConstant.DATE_FORMAT_DATE, SysConfigConstant.DATE_FORMAT_START);
                    endTime = DateUtil.appointedDayStrToFormatStr(endTime, SysConfigConstant.DATE_FORMAT_DATE, SysConfigConstant.DATE_FORMAT_END);
//                    }
                    break;
            }
        }
    ...
    public static List<String> getListDate(String data) {
        ...
        //开始日期
        Date startDate = DateUtil.strToDate(dateLimit.getStartTime(), SysConfigConstant.DATE_FORMAT);

        //结束日期
        Date endDate = DateUtil.strToDate(dateLimit.getEndTime(), SysConfigConstant.DATE_FORMAT);

        while (endDate.after(startDate)){
            date.add(DateUtil.dateToStr(startDate, SysConfigConstant.DATE_FORMAT_DATE)); // 放入集合
            startDate = DateUtil.strToDate(DateUtil.addDay(startDate, 1, SysConfigConstant.DATE_FORMAT), SysConfigConstant.DATE_FORMAT); //循环一次 加一天
        }
    ...
    public static String timestamp2DateStr(Long timestamp, String dateFormat) {
        ...
        if (StrUtil.isBlank(dateFormat)) {
            dateFormat = SysConfigConstant.DATE_FORMAT;
        }
    ...
    public static Long dateStr2Timestamp(String dateStr, String type) {
        ...
        if (StrUtil.isNotBlank(type)) {
            if (type.equals(SysConfigConstant.DATE_TIME_TYPE_BEGIN)) {
                parse = cn.hutool.core.date.DateUtil.beginOfDay(parse);
            }
            if (type.equals(SysConfigConstant.DATE_TIME_TYPE_END)) {
